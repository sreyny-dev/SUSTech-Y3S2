...
public class Version2 {
    ...
    static class Graph {
        ...
        public int findOptimalSubinterval(int s, int[] diameterPath) {
            ...
            int currentWeight = 0;
            int totalWeight = 0;
            //store arr of Accumulated Distance
            int[] accumDistanceList = new int[diameterPath.length];
            accumDistanceList[0] = 0; // Accumulated distance for the first subinterval
            for (int i = 1; i < diameterPath.length; i++) {
                int dis = getAccumulatedDistances(0, i, diameterPath);
                accumDistanceList[i] = dis;
            }
            ...
            while (end < diameterPath.length - 1) {
                if (currentWeight <= s && end < diameterPath.length - 1) {
                    ...
                    if (currentWeight <= s) {
                        ...
                        int b = totalWeight - accumDistanceList[end];
                        result = Math.min(result, Math.max(a, b));
                    }
                }
                if (currentWeight > s && start <= end) {
...
//        accumDistanceList[0] = 0; // Accumulated distance for the first subinterval
//        for (int i = 1; i < diameterPath.length; i++) {
//            int dis = G.getAccumulatedDistances(0, i, diameterPath);
//            accumDistanceList[i] = dis;
//        }
//        for (int j = 0; j < diameterPath.length; j++) {
//            System.out.print(accumDistanceList[j] + " ");
//        }
    }
}
